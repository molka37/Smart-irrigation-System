...
import com.enis.cot.smartirrigation.controllers.Role;
...
public class OAuth2PKCE {
    ...
    // FIX 1: Declare static fields BEFORE static initializer blocks that assign them
    ...
    static {
        ...
        char[] password = config.getValue("jwtKeystorePassword", String.class).toCharArray();
        String alias = config.getValue("jwtAlias", String.class);
        PrivateKey pk = null;
        PublicKey pub = null;
        try {
            KeyStore ks = KeyStore.getInstance("JKS");
            String keystorePath = config.getValue("jwtKeystore", String.class);
            ...
            if (key instanceof PrivateKey) {
                ...
                System.err.println("[OAuth2PKCE] ERROR: No private key found for alias '" + alias + "'");
            }
        } catch (Exception e) {
            System.err.println("[OAuth2PKCE] ERROR loading keystore: " + e.getMessage());
            e.printStackTrace();
        } finally {
            if (fis != null) {
    ...
    public OAuth2PKCE() {
        if (privateKey == null) {
            throw new IllegalStateException(
                    "[OAuth2PKCE] Private key is null. Check keystore, alias and password in config."
            );
        }
    ...
    public String checkCode(String code, String codeVerifier) throws Exception {
        ...
        // FIX 2: Use Base64URL encoding without padding (PKCE spec: RFC 7636)
        // ❌ Old: Base64.getEncoder() + manual substring/replace — wrong and fragile
        // ✅ New: Base64.getUrlEncoder().withoutPadding() — correct per spec
    ...
    public String generateTokenFor(Identity identity) throws Exception {
        ...
        JWSObject jwsObject = new JWSObject(
                new JWSHeader.Builder(JWSAlgorithm.RS512).type(new JOSEObjectType("jwt")).build(),
